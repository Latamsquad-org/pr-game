...
import latamfiles as latfiles
import latamkits as latkits
...
g_adminCommands = {}
g_last_setnext = None
# Tabla aparte: ultimo !setnext de la sesion (NO se limpia al cambiar de mapa; se reescribe).
# {'admin': nombre, 'from_map': mapa donde se ejecuto, 'to_map': destino}
g_setnext_log = {}
g_sponsorText = ''
g_mapList = {}
g_lastPlayedMaps = []
...
g_voteAdmin = ''
# Cola de admins para !mvote (orden de llegada; quien inicia votacion va al final).
g_mvote_admin_queue = []
g_prism_admins = []
g_lastBan = ()
g_announcerTimers = {}
...
# Ultima IP/nombre por hash (connect/disconnect) para !timebanid / !banid offline.
g_lastKnownPlayerByHash = {}
...
class BanSystem():
    ...
    def onPlayerConnect(self, player):
        ...
        if self.isPlayerBanned(player):
            ...
            logAdmin('!kick', 'SERVER', player.getName(), 'DENIED banned Account')
    ...
    def ban_id(self, cmd, banningAdmin):
        ...
        date = time.time()
        playerId = cmd[0]
        knownName, knownIp = resolveBanIdentity(playerId)
        if knownName and knownName != 'MANUALLY BANNED ID':
            name = knownName
        ip = knownIp if knownIp else ''
        if ip == '0.0.0.0':
            ip = ''
        reason = ' '.join(cmd[1:])
        length = None
    ...
    def getLastKnownIdentity(self, playerId):
        # Ultimo nombre e IP no vacios en logs (ignora MANUALLY BANNED ID / IP vacia).
        query = 'SELECT name, ip FROM logs WHERE player_id == ? ORDER BY date DESC LIMIT 30'
        try:
            self.cur.execute(query, (playerId,))
        ...
        for row in rows:
            rowName = row[0]
            rowIp = row[1]
            if name is None and rowName:
                text = str(rowName).strip()
                if text and text != 'MANUALLY BANNED ID':
                    name = text
            if ip is None and rowIp:
                text = str(rowIp).strip()
                if text and text not in ('-', '0.0.0.0') and _looksLikeIpv4(text):
                    ip = text
...
def findReason(reason, ban = False):
    if reason.lower().strip() in ras.adm_reasons:
        reason = ras.adm_reasons[reason.lower().strip()]
    if ban and ras.adm_ban_reason_suffix:
        reason = reason + ' ' + ras.adm_ban_reason_suffix
    return reason
...
def setNextMap(mapId, mapName, gamemode, layer, p):
    ...
    # Mapa actual (donde el admin ejecuto !setnext) para la tabla persistente.
    try:
        from_map = rcore.getMapName(None, True)
    except Exception:
        from_map = '?'
    globalMessage('El siguiente mapa es: %s (%s, %s)' % (mapName, gamemode, layer))
    host.rcon_invoke('admin.nextLevel %i' % mapId)
    # Original: se limpia al cambiar de mapa (onBalanceGameStatusChanged).
    g_last_setnext = p.getName()
    # Tabla aparte: persiste entre mapas; solo se reescribe en cada !setnext.
    g_setnext_log = {
        'admin': p.getName(),
        'from_map': from_map,
        'to_map': '%s (%s, %s)' % (mapName, gamemode, layer),
    }
...
def banPlayer(cmd, banee, banningAdmin, length, reasonArgs):
    ...
    if len(foundPlayers) == 0:
        ...
        reason = findReason(' '.join(reasonArgs), ban=True)
        annocMessage, pbmsg, bantype = getPBBanString(length, reason)
        ...
        for player in foundPlayers:
            if banningAdmin == player:
                adminPM('No puedes banearte a ti mismo!', banningAdmin)
                continue
...
def checkSquadlessPlayers(data = ''):
    ...
    for player in rcore.getPlayers():
        ...
        if nPlayers / maxPlayers < ras.sqd_kickAFKPercent:
            pass
        #Desactivamos el kickeo a admins
        elif ras.sqd_kickSquadLessAFK and AFKDetection.estimateAFKNess(player) >= kick_time and player.isAdmin == False :
        ...
        #Desactivamos el kickeo a admins
        if timenow - timelastchange > ras.sqd_kickSquadLessTime and player.isAdmin == False :
...
def onGameStatusChanged(status):
    ...
    if status == bf2.GameStatus.Loaded:
        ...
        # !history muestra hasta 10 mapas previos de la sesion.
        g_lastPlayedMaps = g_lastPlayedMaps[:10]
        # Turno !mvote: la rotacion ocurre al iniciar votacion (no al fin de ronda).
        mvote_queue_advance_round()
        if g_voteRunning:
...
def onPlayerConnect(p):
    ...
    if ras.log_connects:
        ...
        # Aunque no se loguee connect, cachear hash->nombre/IP para bans offline.
        rememberPlayerEndpoint(p)
...
def onPlayerVerified(p):
    ...
    pHash = realityserver.getPlayerHash(p)
    # Hash ya disponible: guardar nombre/IP para !timebanid offline.
    rememberPlayerEndpoint(p)
    if pHash in ras.adm_adminHashes:
        if adminsOnline == 0 and g_lite_admins:
            adminPM('Un admin se ha unido! Los liteadmin han perdido el poder.', p, display=True)
        g_admins[p] = ras.adm_adminHashes[pHash]
        p.isAdmin = True
        # Cola !mvote: al final por orden de llegada.
        mvote_queue_add_player(p)
    ...
    if not rplayerdata.isPlayerWhitelisted(p):
        ...
        for key in keys:
            if key in ras.whitelisted_player_ids:
                logAdmin('allowedid', 'SERVER', p.getName(), 'ALLOWED Account related to banned key: %s' % key)
                return
            if (key.lower(),) in bannedKeys:
                logAdmin('!kick', 'SERVER', p.getName(), 'DENIED Account related to banned key: %s' % key)
...
def onPlayerDisconnect(player):
    ...
    if player in g_admins:
        mvote_queue_remove_player(player)
        del g_admins[player]
            ...
            if ras.sqd_resignEarly:
                ...
                if ras.sqd_kickLimit > 0:
                    ...
                    if player.failedSquads >= ras.sqd_kickLimit:
                        kickPlayer(player, 'Has creado una escuadra muy temprano! Recuerda esperar al minuto 2:00.', True, None)
                        logAdmin('!kick', 'SERVER', player.getName(), 'Creating too many early squads!')
        return
...
# Capas y modos como se guardan en g_lastPlayedMaps (EndGame).
# Modos con espacios primero para parsear bien "Vehicle Warfare", etc.
_HISTORY_LAYER_ABBRS = frozenset(['std', 'alt', 'lrg', 'inf', 'unknown'])
_HISTORY_MODE_NAMES = (
    'command and control',
    'vehicle warfare',
    'counter-attack',
...
def history_entry_map_name(entry):
    """
    Extrae solo el nombre del mapa de una entrada de historial
    ("Mapa Modo Capa"). Comparacion para !mvote anti-repeticion.
    """
    if entry is None:
        return ''
    ...
    if not text:
        return ''
    low = text.lower()
    # Quitar capa (ultimo token conocido).
    parts = text.rsplit(' ', 1)
    if len(parts) == 2 and parts[1].lower() in _HISTORY_LAYER_ABBRS:
        text = parts[0]
        low = text.lower()
    # Quitar modo (sufijo conocido, mas largo primero).
    for mode in _HISTORY_MODE_NAMES:
        suffix = ' ' + mode
        if low.endswith(suffix):
            return text[:len(text) - len(suffix)].strip()
    # Fallback: todo menos los 2 ultimos tokens.
    parts = text.split(' ')
    if len(parts) >= 3:
        return ' '.join(parts[:-2])
    return text
...
def recent_played_map_names(limit=10):
    """Nombres de mapa (lower) de los ultimos N en g_lastPlayedMaps."""
    names = []
    for entry in list(g_lastPlayedMaps)[:limit]:
        name = history_entry_map_name(entry)
        if name:
            names.append(name.lower())
    return names


# Sufijos en el arg de !mvote que permiten repetir mapa del historial.
_MVOTE_HISTORY_REPEAT_SUFFIXES = ('_veh', '_ski', '_gun')


def mvote_arg_allows_history_repeat(map_arg):
    """
    True si el argumento incluye _veh, _ski o _gun (ej. fallujah_veh).
    Esos modos especiales pueden repetir mapa aunque este en !history.
    """
    text = str(map_arg or '').lower()
    for suffix in _MVOTE_HISTORY_REPEAT_SUFFIXES:
        if text.find(suffix) != -1:
            return True
    return False


def mvote_maps_blocked_by_history(maps, limit=10):
    """
    Retorna lista de nombres display bloqueados (en historial reciente).
    Compara solo por nombre de mapa (ignora modo/capa).
    Args con _veh/_ski/_gun no se bloquean (pueden repetir).
    """
    recent = set(recent_played_map_names(limit))
    if not recent:
        return []
    blocked = []
    seen = set()
    for map_arg in list(maps or []):
        # Excepcion: modos especiales (vehicle/skirmish/gungame) pueden repetir.
        if mvote_arg_allows_history_repeat(map_arg):
            continue
        display = findMapName(map_arg)
        key = str(display).lower().strip()
...
def mvote_normalize_admin_name(name):
    """
    Nombre canonico para unificar cola !mvote (ingame + PRISM).
    lower, strip, quita tag [clan] al inicio, limpia . _ en bordes.
    """
    text = str(name or '').strip().lower()
    if not text:
        return ''
    # Prefijo tipo [KKCK] o [xx]
    if text.startswith('['):
        close = text.find(']')
        if close != -1:
            text = text[close + 1:].strip()
    # BF2 a veces: "CLAN nombre" -> getPlayerName ya toma el ultimo token;
    # aqui igual por si llega el nombre completo.
    if ' ' in text:
        text = text.split(' ')[-1]
    text = text.strip(' ._')
    return text
...
def mvote_queue_key_from_name(name):
    """Clave de cola por nombre canonico: n:chaziz."""
    canon = mvote_normalize_admin_name(name)
    if not canon:
        return None
    return 'n:' + canon


def mvote_queue_key_ingame(p_hash):
    """Compat: resuelve hash ingame a clave n:nombre si el admin esta online."""
    for admin in list(g_admins.keys()):
        try:
            if realityserver.getPlayerHash(admin) == p_hash:
                return mvote_queue_key_for_player(admin)
        except Exception:
            continue
    return None


def mvote_queue_key_prism(admin_name):
    """Clave de cola para admin PRISM (mismo esquema n:nombre)."""
    return mvote_queue_key_from_name(admin_name)


def mvote_queue_key_for_player(player):
    """
    Clave unificada cola !mvote: n:<nombre_canonico>.
    Misma persona ingame + PRISM -> un solo lugar.
    """
    if player is None:
        return None
    try:
        if player.isRcon():
            name = player.getName()
            if name:
                return mvote_queue_key_from_name(name)
    except Exception:
        pass
    try:
        p_hash = realityserver.getPlayerHash(player)
    except Exception:
        return None
    if not p_hash:
        return None
    if p_hash not in ras.adm_adminHashes:
        return None
    try:
...
def mvote_queue_migrate_key(key):
    """Convierte claves viejas h:/p:/hash a n:nombre; None si offline."""
    if not key:
        return None
    key = str(key)
    if key.startswith('n:'):
        return key
    if key.startswith('p:'):
        return mvote_queue_key_from_name(key[2:])
    if key.startswith('h:'):
        return mvote_queue_key_ingame(key[2:])
    # Compat cola muy vieja (solo hash).
    return mvote_queue_key_ingame(key)


def mvote_queue_add_key(key):
    """Agrega una clave al final de la cola si no estaba."""
    global g_mvote_admin_queue
    key = mvote_queue_migrate_key(key) or key
    if not key:
        return
    if not str(key).startswith('n:'):
        return
    if key not in g_mvote_admin_queue:
        g_mvote_admin_queue.append(key)


def mvote_queue_remove_key(key):
    """Quita una clave de la cola."""
    global g_mvote_admin_queue
    key = mvote_queue_migrate_key(key) or key
    if key in g_mvote_admin_queue:
        g_mvote_admin_queue.remove(key)


def mvote_queue_canon_still_online(canon, exclude_player=None, exclude_prism=None):
    """True si queda otra sesion (ingame/PRISM) con el mismo nombre canonico."""
    if not canon:
        return False
    for admin in list(g_admins.keys()):
        if exclude_player is not None and admin is exclude_player:
            continue
        try:
            name = getPlayerName(admin)
        except Exception:
            try:
                name = admin.getName()
            except Exception:
                continue
        if mvote_normalize_admin_name(name) == canon:
            return True
    for name in list(g_prism_admins):
        if exclude_prism is not None and str(name) == str(exclude_prism):
            continue
        if mvote_normalize_admin_name(name) == canon:
            return True
    return False
...
def mvote_queue_add_player(player):
    """Agrega admin (ingame o PRISM) al final de la cola !mvote."""
    mvote_queue_add_key(mvote_queue_key_for_player(player))


def mvote_queue_remove_player(player):
    """
    Saca de la cola al desconectar, solo si no queda la otra sesion
    (ingame vs PRISM) con el mismo nombre.
    """
    key = mvote_queue_key_for_player(player)
    if not key or not key.startswith('n:'):
        return
    canon = key[2:]
    if mvote_queue_canon_still_online(canon, exclude_player=player, exclude_prism=None):
        return
    mvote_queue_remove_key(key)


def mvote_queue_prune():
    """
    Sincroniza cola con admins ingame + PRISM conectados.
    Una entrada por nombre canonico; quita offline; agrega faltantes al final.
    """
    global g_mvote_admin_queue
    online = []
    seen = set()
    # Ingame
    for admin in list(g_admins.keys()):
        key = mvote_queue_key_for_player(admin)
        if not key or key in seen:
            continue
        seen.add(key)
        online.append(key)
    # PRISM (misma clave n: si el nombre coincide)
    for name in list(g_prism_admins):
        key = mvote_queue_key_from_name(name)
        if not key or key in seen:
            continue
        seen.add(key)
        online.append(key)
    # Migrar orden previo (h:/p: -> n:) y descartar offline.
    kept = []
    kept_seen = set()
    for old in list(g_mvote_admin_queue):
        key = mvote_queue_migrate_key(old)
        if not key or key not in seen or key in kept_seen:
            continue
        kept_seen.add(key)
        kept.append(key)
    for k in online:
        if k not in kept_seen:
            kept.append(k)
            kept_seen.add(k)
    g_mvote_admin_queue = kept


def mvote_queue_name_for_key(key):
    """Nombre visible (marca +PRISM si tambien esta por PRISM)."""
    if not key:
        return '?'
    key = mvote_queue_migrate_key(key) or key
    ...
    for admin in list(g_admins.keys()):
        try:
            raw = getPlayerName(admin)
        except Exception:
            try:
                raw = admin.getName()
            except Exception:
                continue
        if mvote_normalize_admin_name(raw) == canon:
            ingame_label = raw
            break
    for name in list(g_prism_admins):
        if mvote_normalize_admin_name(name) == canon:
            prism_label = name
            break
    if ingame_label and prism_label:
        return '%s (+PRISM)' % ingame_label
    if prism_label:
        return '%s (PRISM)' % prism_label
    if ingame_label:
        return ingame_label
    return canon


def mvote_queue_move_to_end(player):
    """
    Quien inicia !mvote pasa al final de la cola (orden de llegada).
    """
    global g_mvote_admin_queue
    key = mvote_queue_key_for_player(player)
    if not key:
        return
    mvote_queue_add_key(key)
    mvote_queue_prune()
    if key in g_mvote_admin_queue:
        g_mvote_admin_queue.remove(key)
        g_mvote_admin_queue.append(key)
...
def mvote_queue_advance_round():
    """
    Fin de ronda: ya no rota automaticamente.
    La rotacion ocurre al iniciar !mvote (mvote_queue_move_to_end).
    """
    mvote_queue_prune()


def format_mvote_queue_message():
    """Texto de cola !mvote list (orden de llegada; quien vota va al final)."""
    mvote_queue_prune()
    if not g_mvote_admin_queue:
        return 'Cola !mvote: (vacia - no hay admins online)'
    lines = [
        'Cola !mvote (orden de llegada; quien inicia !mvote va al final):',
        'Cualquier admin puede iniciar !mvote.',
    ]
    for i, key in enumerate(g_mvote_admin_queue):
        name = mvote_queue_name_for_key(key)
        lines.append('%d. %s' % (i + 1, name))
    return '\r\n'.join(lines)


def send_mvote_queue_to_admin(p):
    """Envia solo la cola de turnos (!mvote list)."""
    sendRhooksAdminWarnEventWrapper(p, format_mvote_queue_message(), history=False)


def send_mvote_last_result_to_admin(p):
    """Envia resumen de la ultima votacion si existe."""
    if g_vote_res_msg != '':
        sendRhooksAdminWarnEventWrapper(
            p,
            'Ultima votacion: ' + g_vote_res_msg + ' %s minutos, en %s'
            % ((int(time.time()) - g_vote_res_time) / 60, g_vote_performed_on),
            history=False,
        )
        return True
    return False
...
def logConnect(player, connect):
    d = time.strftime(ras.log_time_format)
    # Cachea hash->IP/nombre para bans offline (!timebanid / !banid).
    rememberPlayerEndpoint(player)
    if connect:
...
def _looksLikeIpv4(ip):
    # True si parece IPv4 (4 octetos numericos).
...
def rememberPlayerEndpoint(player):
    # Guarda ultima IP y nombre vistos para un hash de jugador.
    global g_lastKnownPlayerByHash
    try:
        playerId = realityserver.getPlayerHash(player)
        if not playerId:
            return
        entry = g_lastKnownPlayerByHash.get(playerId, {})
        try:
            name = player.getName()
            if name:
                entry['name'] = name
            ...
            pass
        try:
            ip = str(player.getAddress()).strip()
            ...
            pass
        if entry:
            g_lastKnownPlayerByHash[playerId] = entry
    except:
        return


def _banLogPath():
    # Ruta de banlist_info.log segun realityconfig_admin.
    try:
        modDir = host.sgl_getModDirectory()
        base = ras.log_bans_path.replace('[MOD]', modDir)
        return base + ras.log_bans_file
    except:
        return None


def lookupBanLogIdentity(playerId):
    # Busca hacia atras en banlist_info.log el ultimo nombre/IP de ese hash.
    path = _banLogPath()
    if not path:
        return (None, None)
    try:
        ...
        try:
            handle.seek(0, 2)
            size = handle.tell()
            handle.seek(max(0, size - 262144), 0)
            data = handle.read()
        finally:
            handle.close()
    except:
    ...
    while i >= 0:
        ...
        if pos < 0:
            continue
        rest = line[pos + len(needle):].strip()
        ipMatch = re.search(r'(?:^|\s)(\d{1,3}(?:\.\d{1,3}){3}|-|0\.0\.0\.0)(?:\s|$)', rest)
        if not ipMatch:
            continue
        name = rest[:ipMatch.start(1)].strip()
        ipRaw = ipMatch.group(1)
        if name == 'MANUALLY BANNED ID':
            continue
        ip = None
        if ipRaw not in ('-', '0.0.0.0') and _looksLikeIpv4(ipRaw):
            ip = ipRaw
        if name:
            return (name, ip)
    return (None, None)
...
def resolveBanIdentity(playerId):
    # Nombre e IP: cache sesion -> banlist_info.log -> bans.sqlite3 -> placeholders.
    name = 'MANUALLY BANNED ID'
    ip = '0.0.0.0'
    cached = g_lastKnownPlayerByHash.get(playerId)
    if cached:
        if cached.get('ip') and _looksLikeIpv4(cached.get('ip')):
            ip = cached['ip']
        if cached.get('name'):
            name = cached['name']
    if ip == '0.0.0.0' or name == 'MANUALLY BANNED ID':
        logName, logIp = lookupBanLogIdentity(playerId)
        if name == 'MANUALLY BANNED ID' and logName:
            name = logName
        if ip == '0.0.0.0' and logIp:
            ip = logIp
    if g_banSystem is not None and (ip == '0.0.0.0' or name == 'MANUALLY BANNED ID'):
        dbName, dbIp = g_banSystem.getLastKnownIdentity(playerId)
        if ip == '0.0.0.0' and dbIp:
            ip = dbIp
        if name == 'MANUALLY BANNED ID' and dbName:
            name = dbName
    return (name, ip)
...
def logBan(player, reason, length, banningAdmin):
    # Escribe banlist_info.log: fecha, hash, nombre, IP, motivo, admin, duracion y mapa.
    if not ras.log_bans:
        return
    if 'BanLog' not in rlogger.RealityLogger:
        return
    adminName = banningAdmin.getName() if banningAdmin else 'NO ADMIN FOUND'
    logBanId(
...
def logBanId(playerId, playerName, playerIp, reason, length, adminName):
    # Misma linea que logBan, para !timebanid / !banid (jugador puede estar offline).
    if not ras.log_bans:
        return
    if 'BanLog' not in rlogger.RealityLogger:
        return
    d = time.strftime(ras.log_date_format)
    name = playerName if playerName else 'MANUALLY BANNED ID'
    ip = playerIp if playerIp else '0.0.0.0'
    # Si falta nombre/IP, completar con la ultima identidad conocida del hash.
    if name == 'MANUALLY BANNED ID' or not ip or ip in ('-', '0.0.0.0'):
        resolvedName, resolvedIp = resolveBanIdentity(playerId)
        if name == 'MANUALLY BANNED ID' and resolvedName:
            name = resolvedName
        if (not ip or ip in ('-', '0.0.0.0')) and resolvedIp and resolvedIp != '0.0.0.0':
            ip = resolvedIp
    admin = adminName if adminName else 'NO ADMIN FOUND'
    try:
        mapName = rcore.getMapName()
    except:
        mapName = '-'
    rlogger.RealityLogger['BanLog'].logLine('[%s] %s %s %s %s banned by %s (%s) (%s)' % (
        d,
        playerId,
        name,
        ip,
...
def onBalanceGameStatusChanged(status):
    ...
    if status == bf2.GameStatus.Playing:
        ...
        g_switch_next.clear()
        g_last_setnext = None
        # g_setnext_log NO se limpia: persiste entre mapas.
    return
...
def prismAdminConnected(adminName):
    ...
    # Misma cola !mvote: clave n:nombre (unifica con ingame homonimo).
    mvote_queue_add_key(mvote_queue_key_from_name(adminName))


def prismAdminDisconnected(adminName):
    if adminName in g_prism_admins:
        g_prism_admins.remove(adminName)
    key = mvote_queue_key_from_name(adminName)
    if not key:
        return
    canon = key[2:]
    # Solo sacar si no queda sesion ingame con el mismo nombre.
    if mvote_queue_canon_still_online(canon, exclude_player=None, exclude_prism=adminName):
        return
    mvote_queue_remove_key(key)
...
def fillCommandDict():
    g_adminCommands.update({'aa': commandAutoadmin,
        ...
        'assetban': commandAssetBan,
        'assetunban': commandAssetUnban,
        'tp': commandTeleport,
        'tpto': commandTeleportTo,
        'heal': commandHeal,
        'rearm': commandRearm,
        'push': commandPush,
        'addvip': commandAddVip,
        'pilot': commandPilot
        })
...
def commandBanId(args, p):
    ...
    if g_banSystem.ban_id(args, p):
        ...
        banName, banIp = resolveBanIdentity(args[0])
        reason = findReason(' '.join(args[1:]), ban=True)
        for player in rcore.getPlayers():
            if realityserver.getPlayerHash(player) == args[0]:
                ...
                break
        # banlist_info.log: !banid tampoco pasaba por logBan
        logBanId(args[0], banName, banIp, reason, 'perm', p.getName())
        return True
...
def commandTimeBanId(args, p):
    if len(args) < 3:
        ...
        reason = findReason(' '.join(args[2:]), ban=True)
        banName, banIp = resolveBanIdentity(args[0])
        for player in rcore.getPlayers():
            if realityserver.getPlayerHash(player) == args[0]:
                banName = player.getName()
                ...
                rememberPlayerEndpoint(player)
                break
        dbIp = banIp if banIp and banIp != '0.0.0.0' else ''
        try:
            if g_banSystem._enter_log(banName, None, time.time(), args[0], dbIp, reason, 'timeban', length, p.getName(), g_banSystem.round_start):
                logAdmin('!timebanid', p.getName(), 'id %s sucessfully banned' % args[0], reason)
                personalMessage('PlayerID %s exitosamente timebaneado.' % args[0], p)
                # banlist_info.log: !timebanid no pasa por banPlayer/logBan
                for player in rcore.getPlayers():
                    if realityserver.getPlayerHash(player) == args[0]:
                        kickPlayer(player, reason, announce=True, admin=p, bansystem=True)
                        break
                logBanId(args[0], banName, banIp, reason, length, p.getName())
                return True
        except sqlite3.IntegrityError:
...
def commandHistory(args, p):
    msgstr = ''
    if len(g_lastPlayedMaps) == 0:
        sendRhooksAdminWarnEventWrapper(p, 'No hay mapas en el historial.', history=False)
        return True
...
def commandInit(args, p):
    ...
    # Mantener orden de cola existente; sincronizar con admins actuales.
    mvote_queue_prune()
...
def commandMapVote(args, p):
    ...
    if pHash in ras.adm_adminHashes or p.isRcon():
        # !mvote list - orden de llegada (quien inicia !mvote va al final).
        if len(args) == 1 and str(args[0]).lower() == 'list':
            send_mvote_queue_to_admin(p)
            return True
        ...
        if g_voteRunning:
            if len(args) == 1 and args[0] == 'cancel':
                ...
                return True
            if len(args) == 0:
                send_mvote_last_result_to_admin(p)
                sendRhooksAdminWarnEventWrapper(
                    p,
                    "Votacion en progreso. Escribe '!mvote cancel' para cancelarla. Cola: !mvote list",
                    history=False,
                )
                return True
            else:
            ...
            # Sin args: ultima votacion / ayuda (cola en !mvote list).
            if not send_mvote_last_result_to_admin(p):
                sendRhooksAdminWarnEventWrapper(
                    p,
                    'Aun no ha habido una votacion en esta ronda.',
                    history=False,
                )
            return True
        else:
            if len(args) < 2 or len(args) > 4:
                ...
                return False
            # Cualquier admin puede iniciar !mvote (cola solo ordena por llegada).
            # Anti-repeticion: no iniciar si algun mapa esta en los ultimos 10.
            # Excepcion: args con _veh/_ski/_gun pueden repetir ese mapa.
            blocked = mvote_maps_blocked_by_history(args, 10)
            if blocked:
                sendRhooksAdminWarnEventWrapper(
                    p,
                    'No se puede iniciar la votacion: mapa(s) en los ultimos 10 jugados: %s'
                    % ', '.join(blocked),
                    history=False,
                )
                return False
            initializeVote(args, p)
            # Quien vota pasa al final de la cola.
            mvote_queue_move_to_end(p)
            return True
    else:
        if g_vote_res_msg != '':
...
def commandReload(args, p):
    ...
    globalMessage('Reiniciando mapa. - %s' % p.getName())
...
def format_setnext_log_message():
    """
    Texto desde g_setnext_log (tabla persistente entre mapas).
    Retorna None si nadie uso !setnext desde que arranco el server.
    """
    if not g_setnext_log:
        return None
    admin = g_setnext_log.get('admin')
    if not admin:
        return None
    from_map = g_setnext_log.get('from_map') or '?'
    return 'El ultimo setnext lo hizo %s en %s' % (admin, from_map)


def commandSetNext(args, p):
    if len(args) < 1:
        personalMessage('Por favor especifica el nombre de un mapa.', p)
        # Aviso HUD desde tabla persistente (admin + mapa de origen).
        msg = format_setnext_log_message()
        if msg:
            sendRhooksAdminWarnEventWrapper(p, msg, history=False)
        return False
...
def commandUnban(args, p):
    global g_lastBan
    if not g_lastBan:
        personalMessage('No se encontro el jugador.', p)
        return True
    ...
    if g_banSystem.remove_ban(g_lastBan[1]):
        personalMessage('El jugador %s ha sido desbaneado.' % g_lastBan[0], p)
...
def addCommand(name, handler, permissions, allowOverride = False):
    ...
    # Config manda: no pisar adm_adminPowerLevels si el comando ya esta ahi
    if name not in ras.adm_adminPowerLevels:
        ras.adm_adminPowerLevels[name] = permissions
    rdebug.debugMessage(
        'Command %s mapped to %s, permissions: %s'
        % (name, str(handler), ras.adm_adminPowerLevels.get(name, permissions)),
        'admin',
    )
    return True
            ...
            if self.hasMultiple():
                if ras.adm_notifySameIP:
                    for player in self.players:
                        if player != Player:
                            adminPM('El jugador ' + Player.getName() + ' tiene conflico de IP con ' + player.getName())
...
def commandAssetBan(args, p):
    latassban.assetBan(args, p)
    
def commandAssetUnban(args, p):
    latassban.assetUnban(args, p)
    
def commandAddVip (args, p):
    if len(args) != 1:
        personalMessage('Por favor especifica el nombre exacto de un jugador para dar acceso VIP.', p)
        return False
    else:
        reservedSlotsPath = '%s/settings/reservedslots.con' % host.sgl_getModDirectory()
        with open(reservedSlotsPath, 'r') as readRes:
            if str(args[0]) in readRes.read():
                adminPM ('El jugador ' + str(args[0]) + ' ya se encuentra en la lista de acceso VIP.')
                return False
        with open(reservedSlotsPath, 'a') as appendRes:
            appendRes.write(''.join(['\nreservedSlots.addNick "' + args[0] + '"']))
            #reserved.writelines(['reservedSlots.addNick "' + args[0] + '"'])
        host.rcon_invoke('reservedSlots.addNick %s' % args[0])
        adminPM ('El jugador ' + str(args[0]) + ' ha sido agreado a la lista de acceso VIP.')
        return True
...
def commandPilot(args, p):
    team = p.getTeam()
    if latkits.pilotKit(p,team) == True:   
