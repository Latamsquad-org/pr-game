<?php
declare(strict_types=1);
require_once __DIR__ . '/auth/lib.php';
auth_start_session();
$id = auth_current_discord_id();
$name = auth_display_name();
$isStaff = $id !== null && auth_is_staff();
$logoV = (string) @filemtime(__DIR__ . '/assets/img/latamfiles-logo.png');
$cssV = (string) @filemtime(__DIR__ . '/assets/css/site.css');
$origin = 'https://latamsquad.dev';
$canonical = $origin . '/';
$ogImage = $origin . '/assets/img/latamfiles-logo.png';
$title = 'LATAMFILES — Archivos y demos LATAMSQUAD';
$description = 'LATAMFILES: demos 2D/3D, tracker de rondas y archivos de Project Reality de la comunidad LATAMSQUAD.';
$jsonLd = [
    '@context' => 'https://schema.org',
    '@type' => 'WebSite',
    'name' => 'LATAMFILES',
    'url' => $canonical,
    'description' => $description,
    'inLanguage' => 'es',
    'publisher' => [
        '@type' => 'Organization',
        'name' => 'LATAMSQUAD',
        'url' => 'https://latamsquad.org',
    ],
];
$jsonLdEncoded = json_encode(
    $jsonLd,
    JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_AMP
);
?>
<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="utf-8">
  <title><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></title>
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <meta name="description" content="<?= htmlspecialchars($description, ENT_QUOTES, 'UTF-8') ?>">
  <meta name="robots" content="index,follow">
  <link rel="canonical" href="<?= htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8') ?>">
  <meta name="theme-color" content="#6b8f3c">
  <meta property="og:type" content="website">
  <meta property="og:site_name" content="LATAMFILES">
  <meta property="og:locale" content="es_LA">
  <meta property="og:title" content="<?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?>">
  <meta property="og:description" content="<?= htmlspecialchars($description, ENT_QUOTES, 'UTF-8') ?>">
  <meta property="og:url" content="<?= htmlspecialchars($canonical, ENT_QUOTES, 'UTF-8') ?>">
  <meta property="og:image" content="<?= htmlspecialchars($ogImage, ENT_QUOTES, 'UTF-8') ?>">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="<?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?>">
  <meta name="twitter:description" content="<?= htmlspecialchars($description, ENT_QUOTES, 'UTF-8') ?>">
  <meta name="twitter:image" content="<?= htmlspecialchars($ogImage, ENT_QUOTES, 'UTF-8') ?>">
  <link rel="icon" href="/assets/img/favicon.png" type="image/png">
  <link rel="preconnect" href="https://fonts.googleapis.com">
  <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
  <link href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@500;600;700&display=swap" rel="stylesheet">
  <link rel="stylesheet" href="/assets/css/site.css?v=<?= htmlspecialchars($cssV !== '' ? $cssV : '1', ENT_QUOTES, 'UTF-8') ?>">
  <?php if (is_string($jsonLdEncoded) && $jsonLdEncoded !== ''): ?>
  <script type="application/ld+json"><?= $jsonLdEncoded ?></script>
  <?php endif; ?>
</head>
<body>
  <header class="latam-site-header" role="banner">
    <div class="latam-site-header__inner">
      <a class="latam-site-brand" href="/" aria-label="LATAMFILES — Inicio">
        <img
          class="latam-site-brand__logo"
          src="/assets/img/latamfiles-logo.png?v=<?= htmlspecialchars($logoV !== '' ? $logoV : '1', ENT_QUOTES, 'UTF-8') ?>"
          alt="LATAMFILES — logo LATAMSQUAD"
          width="625"
          height="91"
        >
      </a>
      <div class="latam-site-header__auth">
        <nav class="latam-ext-nav" aria-label="Enlaces comunidad">
          <a class="latam-ext-nav__link latam-ext-nav__link--external latam-ext-nav__link--latamsquad" href="https://latamsquad.org" target="_blank" rel="noopener noreferrer"><span class="latam-ext-nav__latam">LATAM</span><span class="latam-ext-nav__squad">SQUAD</span></a>
          <a class="latam-ext-nav__link latam-ext-nav__link--external latam-ext-nav__link--latamstats" href="https://stats.latamsquad.org/" target="_blank" rel="noopener noreferrer"><span class="latam-ext-nav__latam">LATAM</span><span class="latam-ext-nav__stats">STATS</span></a>
          <a class="latam-ext-nav__link latam-ext-nav__link--external latam-ext-nav__link--latamtorneos" href="https://torneos.latamsquad.org/" target="_blank" rel="noopener noreferrer"><span class="latam-ext-nav__latam">LATAM</span><span class="latam-ext-nav__torneos">TORNEOS</span></a>
          <a class="latam-ext-nav__link latam-ext-nav__link--external latam-ext-nav__link--discord" href="https://discord.gg/latamsquad" target="_blank" rel="noopener noreferrer">Discord</a>
        </nav>
        <?php if ($id === null): ?>
          <a class="latam-site-header__admins" href="/auth/discord.php">Entrar</a>
        <?php else: ?>
          <?= auth_account_chip_html((string) ($name ?? 'Usuario'), $isStaff ? '/admin/' : null) ?>
          · <a href="/auth/logout.php">Salir</a>
        <?php endif; ?>
      </div>
    </div>
  </header>

  <?php /* Hub: mismo ancho que la grilla de juegos para alinear titulo/texto con botones */ ?>
  <main class="latam-main latam-main--hub">
    <h1>Archivos LATAMSQUAD</h1>
    <p>Descargas de demos 2D/3D, tracker de rondas y archivos de la comunidad LATAMSQUAD. Elegi un juego para continuar.</p>
    <section class="game-picker" aria-label="Seleccion de juego">
      <h2 class="game-picker__title">Selecciona un juego</h2>
      <ul class="game-picker__grid">
<?php
$games = [
    ['label' => 'ARMA 3', 'href' => null, 'logo' => 'arma3.png'],
    ['label' => 'ARMA Reforger', 'href' => null, 'logo' => 'arma-reforger.png'],
    ['label' => 'SQUAD', 'href' => null, 'logo' => 'squad.png'],
    ['label' => 'Project Reality', 'href' => '/pr.php', 'logo' => 'project-reality.png'],
    ['label' => 'Realitymod BF3', 'href' => null, 'logo' => 'realitymod-bf3.png'],
    ['label' => 'Forgotten Hope 2', 'href' => null, 'logo' => 'forgotten-hope-2.png'],
    ['label' => 'Multi Theft Auto', 'href' => null, 'logo' => 'multi-theft-auto.png'],
];
foreach ($games as $game) {
    $label = (string) $game['label'];
    $logo = (string) $game['logo'];
    $logoPath = __DIR__ . '/assets/img/games/' . $logo;
    $logoUrl = is_file($logoPath)
        ? '/assets/img/games/' . rawurlencode($logo) . '?v=' . (string) filemtime($logoPath)
        : null;
    $logoClass = 'game-picker__logo';
    if ($logo === 'project-reality.png') {
        $logoClass .= ' game-picker__logo--pr';
    } elseif ($logo === 'multi-theft-auto.png') {
        $logoClass .= ' game-picker__logo--mta';
    }
    $labelEsc = htmlspecialchars($label, ENT_QUOTES, 'UTF-8');
    echo "        <li class=\"game-picker__item\">
";
    if ($game['href'] !== null) {
        $hrefEsc = htmlspecialchars((string) $game['href'], ENT_QUOTES, 'UTF-8');
        echo "          <a class=\"game-picker__btn game-picker__btn--active\" href=\"{$hrefEsc}\">
";
        if ($logoUrl !== null) {
            $srcEsc = htmlspecialchars($logoUrl, ENT_QUOTES, 'UTF-8');
            echo "            <img class=\"{$logoClass}\" src=\"{$srcEsc}\" alt=\"Logo {$labelEsc}\" width=\"160\" height=\"80\" loading=\"lazy\">
";
        }
        echo "            <span class=\"game-picker__label\">{$labelEsc}</span>
";
        echo "            <span class=\"game-picker__soon game-picker__soon--spacer\" aria-hidden=\"true\">Proximamente</span>
";
        echo "          </a>
";
    } else {
        echo "          <button type=\"button\" class=\"game-picker__btn game-picker__btn--disabled\" disabled>
";
        if ($logoUrl !== null) {
            $srcEsc = htmlspecialchars($logoUrl, ENT_QUOTES, 'UTF-8');
            echo "            <img class=\"{$logoClass}\" src=\"{$srcEsc}\" alt=\"Logo {$labelEsc}\" width=\"120\" height=\"48\" loading=\"lazy\">
";
        }
        echo "            <span class=\"game-picker__label\">{$labelEsc}</span>
";
        echo "            <span class=\"game-picker__soon\">Proximamente</span>
";
        echo "          </button>
";
    }
    echo "        </li>
";
}
?>
      </ul>
    </section>
  </main>
  <?php latam_render_footer(); ?>
</body>
</html>
