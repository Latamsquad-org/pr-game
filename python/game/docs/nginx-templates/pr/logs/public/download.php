...
function saveHashFiles( $server ){
    ...
    // master file (joinlog.log en PR moderno; cdhash.txt suele estar vacio)
    ...
    // incremental file
    $getIncremental = '';
    if( !empty($server['cdhash_main']) ) {
        $inc = @file_get_contents( $server['cdhash_main'] );
        if ($inc !== false) {
            $getIncremental = $inc;
        }
    }
    ...
    // save file
    file_put_contents(__DIR__ . '/logs/hash_'.$server['local_name'],$masterFile.$getIncremental);
