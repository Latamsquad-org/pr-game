...
Application.controller('ApplicationController',['$scope','$filter','$http','$interval',function($scope,$filter,$http,$interval){
    ...
    // Paginacion en cliente: evita congelar el DOM con miles de filas
    $scope.logPageSize = 200;
    $scope.logVisible = 200;
    $scope.playerLogVisible = 200;
    $scope.filteredLogs = [];

    $scope.resetLogPaging = function() {
        $scope.logVisible = $scope.logPageSize;
    };
    ...
    $scope.resetPlayerLogPaging = function() {
        $scope.playerLogVisible = $scope.logPageSize;
    };

    $scope.loadMorePlayerLogs = function() {
        $scope.playerLogVisible += $scope.logPageSize;
    };

    // Al filtrar, volver a la primera tanda
    $scope.$watch('filter_list', function() {
        $scope.resetLogPaging();
    });
