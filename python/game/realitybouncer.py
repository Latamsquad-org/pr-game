...
class ACSYS:
    ...
    def deinit(self):
        ...
        host.unregisterHandler(self.onExitVehicle)
        ...
        for asset in ras.acsys_assets:
            for searchstr in ras.acsys_assets[asset]['squadname_contains']:
                if searchstr.lower() not in name.lower():
                    continue
                if assetFound:
                    radmin.personalMessage('Has sido resignado por crear una escuadra con un nombre ambiguo/confuso.', player)
                ...
                if asset in self.asset_squads[team]:
                    ...
                    radmin.personalMessage('Has sido resignado por crear una escuadra duplicada.', player)
                    rtimer.fireNextTick(radmin.resignPlayer, data=player)
                    return
    ...
    def forceDriverExit(self, data):
        ...
        if not player.isValid():
            ...
            if latassban.isPlayerAssetBanned(player, veh_type) == False and vehicleOwner is None or (player.getTeam(), player.getSquadId()) == vehicleOwner:
                rdebug.debugMessage('ACSYS player is allowed to use vehicle, owner: %s' % str(vehicleOwner), 'acsys')
                return
            if player.isValid() and self.isPlayerDriver(player, vehicle):
    ...
    def onEnterVehicleCheckLowPop(self, player, vehicle, freeSoldier = False):
        ...
        if vtype in self.low_pop_types or vehicle.templateName.lower() in self.low_pop_includes:
            radmin.personalMessage('Se necesitan mas jugadores para poder usar este tipo de vehiculo!', player)
            rmemory.HudVarWriteEventWstringWithTimedShowvar(player, 'PythonGameWarning', 'DEJA DE HACER ESO!\nADVERTENCIA:\nSe necesitan mas jugadores para poder usar este tipo de vehiculo!', 8)
    ...
    def onEnterVehicle(self, player, vehicle, freeSoldier = False):
        ...
        if not ras.acsys_enable:
            ...
            return
        #elif rplayerdata.getPlayerTrustLevel(player) == 0 and rcore.getGameModeName() != 'Vehicle Warfare' and str(rcore.getMapName()) != 'test_airfield':
        #    if veh_type == 5 or veh_type == 7 or veh_type == 14 or veh_type == 1 or veh_type == 4 or veh_type == 2 or veh_type == 3:
        #        rcore.blackScreen(player, fast=True)
        #        rmemory.HudVarWriteEventWstringWithTimedShowvar(player, 'PythonGameWarning', 'DEJA DE HACER ESO!\nADVERTENCIA:\nEl nivel de tu cuenta no es lo suficiente para usar este tipo de vehiculo!\nNivel requerido: 1', 8)
        #        rtimer.fireOnce(self.forceDriverExit, 1.0, data=(player,
        #        vehicle,
        #        0,
        #        self.KICK_ENTER_GRACE))
        elif latassban.isPlayerAssetBanned(player, veh_type) == True and rcore.getGameModeName() != 'Vehicle Warfare':
            rcore.blackScreen(player, fast=True)
            rmemory.HudVarWriteEventWstringWithTimedShowvar(player, 'PythonGameWarning', 'DEJA DE HACER ESO!\nADVERTENCIA:\nLa administracion te ha prohibido usar este tipo de vehiculo!', 8)
            rtimer.fireOnce(self.forceDriverExit, 1.0, data=(player,
             vehicle,
             0,
             self.KICK_ENTER_GRACE))
            ...
            if owner is None:
                ...
                return
            radmin.personalMessage('No estas en la escuadra correcta para usar este tipo de vehiculo!', player)
            rmemory.HudVarWriteEventWstringWithTimedShowvar(player, 'PythonGameWarning', 'ADVERTENCIA:\nNo estas en la escuadra correcta para usar este tipo de vehiculo!', 8)
            rtimer.fireOnce(self.forceDriverExit, 1.0, data=(player,
             vehicle,
             0,
             self.KICK_ENTER_GRACE))
    ...
    def onPlayerChangedSquad(self, player, oldSquadID, newSquadID):
        if not player.isValid():
            ...
            if not self.isPlayerDriver(player, vehicle):
                return
            radmin.personalMessage('No estas en la escuadra correcta para poder usar este tipo de vehiculo!', player)
