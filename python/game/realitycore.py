...
# ID del cliente que en ingles muestra "STOP DOING THAT".
MSG_STOP_DOING_THAT_ID = 1220104
# Texto forzado en espanol (reemplaza el medal event bilingue).
MSG_DEJA_DE_HACER_ESO = 'DEJA DE HACER ESO!'
MSG_DEJA_DE_HACER_ESO_SEC = 5


def sendDejaDeHacerEso(player, seconds=None):
    """Aviso HUD fijo en espanol (antes: STOP DOING THAT via 1220104)."""
    if player is None:
        return False
    if seconds is None:
        seconds = MSG_DEJA_DE_HACER_ESO_SEC
    try:
        rmemory.HudVarWriteEventWstringWithTimedShowvar(
            player,
            'PythonGameWarning',
            MSG_DEJA_DE_HACER_ESO,
            int(seconds),
        )
        return True
    except Exception:
...
def sendMessageToPlayer(player, p_id, level = 0):
    if player and not player.dead:
        # Forzar espanol: no usar el string localizado del cliente (STOP DOING THAT).
        try:
            if int(p_id) == MSG_STOP_DOING_THAT_ID:
                sendDejaDeHacerEso(player)
                return
        except Exception:
            pass
    ...
    if spawngroup <= 0:
        ...
        if rally is not None:
            if not player.allowSpawnOnRally:
                ...
                radmin.personalMessage('You joined the squad while dead, you cannot use its rally point before you spawn', player)
                return False
            if rally.squad != player.getSquadId() and rally.squad != 0:
                ...
                radmin.personalMessage("You selected squad's %s rallypoint, you cannot spawn here." % rally.squad, player)
                return False
        return True
