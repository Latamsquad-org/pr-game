...
PRDEBUG_QAs_ONLINE = []
DEBUG = '\xc2\xa7C1001*DBG* \xc2\xa7C1001'
DEBUG_CONSOLE = 'DBG '
...
def onRemoteCommandPhysics(player, cmd, args):
    ...
    def refresh(args = None):
        if physicsTarget is None or not physicsTarget.isValid() or physicsTarget.getPosition() == (0.0, 0.0, 0.0):
            return
        else:
            physics = realitymemory._getObjectPhysics(physicsTarget)
            if hasattr(physics, 'vx'):
                v = (round(physics.vx, 2), round(physics.vy, 2), round(physics.vz, 2))
            else:
                v = None
            if hasattr(physics, 'rx'):
                rv = (round(physics.rx, 2), round(physics.ry, 2), round(physics.rz, 2))
            else:
                rv = None
            sleepiness = getattr(physics, 'sleepiness', None)
            sleepinessMax = getattr(physics, 'sleepinessMax', None)
            if physicsPrint:
                debugMessage('v: %s, rv: %s, S: %s/%s' % (str(v),
                 str(rv),
                 sleepiness,
                 sleepinessMax))
    ...
    try:
        if args[0] == 'select':
            ...
            physicsSpeed = (float(args[1]), float(args[2]), float(args[3]))
            debugMessage('speed: %s' % physicsPrint)
        elif args[0] == 'rot':
            physicsRotationalVelocity = (float(args[1]), float(args[2]), float(args[3]))
...
def onRemotePosition(player, cmd, args):
    ...
    try:
        if args[0] == 'save':
            ...
            debugMessage('Saved position as %s' % name)
            return
    except:
        pass
    ...
    try:
        ...
        debugMessage('Position: %s,%s,%s' % veh.getPosition())
...
def onRemoteDebugCommand(player, cmd, args):
    ...
    if len(args) == 0:
        if PRDEBUG is not None:
            host.rcon_invoke('game.sayAll "' + DEBUG + 'disabled..."')
            realityadmin.sendToPrism('debugOff', [None])
            PRDEBUG = None
        else:
            host.rcon_invoke('game.sayAll "' + DEBUG + 'enabled..."')
        ...
        for arg in args:
            if arg == 'all':
                ...
                host.rcon_invoke('game.sayAll "' + DEBUG + 'all debugs enabled..."')
            elif arg in PRDEBUG_AVAILABLE:
                if PRDEBUG is not None and arg in PRDEBUG:
                    host.rcon_invoke('game.sayAll "' + DEBUG + arg + ' debug disabled..."')
                    PRDEBUG.remove(arg)
                else:
                    host.rcon_invoke('game.sayAll "' + DEBUG + arg + ' debug enabled..."')
                    if PRDEBUG is None:
                        PRDEBUG = []
                    PRDEBUG.append(arg)
