...
class Progression(object):
    ...
    def __getitem__(self, level):
        ...
        """
        # Clamp: al ganar, level puede ser == len(levels) (max_level).
        if not self.levels:
            raise IndexError('gungame progression is empty')
        try:
            idx = int(level)
        except Exception:
            idx = 0
...
class PRGunGame(cq.PRAAS):
    ...
    def get_level_for_player(self, player):
        # type: (Player) -> Level
        # Usar Progression.__getitem__ (ya clampea) para no crashear si
        # player_state.level == max_level tras ganar la partida.
        return self.progression[self.player_states[player].level]

    def clamp_player_level(self, player_state):
        """Ajusta level al rango valido [0, max_level] (max_level = victoria)."""
        if player_state is None:
            return
        try:
            lvl = int(player_state.level)
        except Exception:
            lvl = 0
        if lvl < 0:
            lvl = 0
        if self.max_level > 0 and lvl > self.max_level:
            lvl = self.max_level
        player_state.level = lvl
    ...
    def enforce_kit(self, player, kit=None):
        ...
        rdebug.debugMessage("Enforcing kit %s (default_idx %s) for player %s" % (kit.name, kit.primary_item, player.getName()))
    ...
    def update_state(self, _data=None):
        for player in self.players_requiring_update:
            ...
            self.clamp_player_level(player_state)

            # Tras ganar, level == max_level: no hay Level de promote/demote.
    ...
    def player_won(self, player):
        ...
        radmin.globalMessage(format_name(player.getName()) + " es el ganador!", True, True)
    ...
    def gg_onPlayerKilled(self, victim, attacker, weapon, assists, _soldier):
        ...
        if assists is not None:
            for assistee in assisting_players(assists):
                ...
                assistee_state = self.player_states.get(assistee)
                if assistee_state is None:
                    continue
    ...
    def gg_onPlayerConnect(self, player):
        ...
        self.clamp_player_level(self.player_states[player])
    ...
    def gg_onPlayerDisconnect(self, player):
        ...
        if player is None:
            return
        # Puede llegar disconnect sin connect previo (o doble disconnect).
        old_player_state = self.player_states.pop(player, None)
        if old_player_state is None:
            return
        try:
            name = player.getName()
        except Exception:
            name = None
        if name:
            self.old_player_states[name] = old_player_state
        self.mark_player_for_update(player)
